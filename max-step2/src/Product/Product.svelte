...
{#if showModal}
<Modal content="modal" on:cancel={() => showModal = false} on:close={()=> showModal = false} let:didAgree={closeAble}> 
    <h1 slot="header">modal</h1>
    <p>text</p>
    <button slot="footer" on:click={() => showModal = false} disabled={!closeAble}>close2</button>
</Modal>
{/if}
